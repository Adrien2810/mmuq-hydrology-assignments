...
def main():
    ...
    boxplot_parameters_results(PARAM_NAMES, CalibratedParameters, Best_Parameter_Assignment1, str(data_dir / "boxplot_parameters_results.png"))
...
def CalibrateModel(precipitation, ags, model):
    ...
    res = differential_evolution(
        ...
        popsize=popsize,
        tol=1e-3,
        mutation=(0.6, 1), # Increased upper bound for mutation to encourage more exploration
...
def boxplot_parameters_results(parameter_name, parameter_values, ref_parameter, out_png):
    """
    Boxplot of TRUE parameter distributions with observation points
    and reference parameter overlay.
    """

    n_params = len(parameter_name)
    n_sims = len(parameter_values)

    # reshape: list of arrays, one per parameter
    # We use the raw 'data' directly instead of 'data_norm'
    data = [
        np.array([parameter_values[i][j] for i in range(n_sims)])
    ...
    # scatter points (jittered raw values)
    ...
    # reference parameters (using raw ref_parameter values)
    ax.scatter(
        np.arange(1, n_params + 1),
        ref_parameter,
        color='red',
        marker='D', # Changed to Diamond to make it pop against the black dots
        s=60,
    ...
    # axis formatting
    ax.set_xticks(np.arange(1, n_params + 1))
    ax.set_xticklabels(parameter_name, rotation=45, ha='right')
    
    # Updated labels and removed y-limits
    ax.set_ylabel("Parameter value (True scale)")
    ax.set_xlabel("Parameters")
    ax.set_title("Parameter distributions (True values)")
    
    # Optional: Use log scale if values vary by orders of magnitude
    # ax.set_yscale('log') 
    ...
    plt.tight_layout()
    plt.savefig(out_png, dpi=200, bbox_inches="tight")
    print(f"Saved boxplot to: {os.path.abspath(out_png)}")
    plt.close(fig)
