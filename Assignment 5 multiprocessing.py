...
def main():
    ...
    OFV_REF = []
    for j in range(Nsim):
        i, best_params, best_ofv, best_metrics, best_sim, ddho_new, diso_new, ofv_old = result[j]
    ...
    if Perturbed_discharge:
        plot_cdf_variable(optn_args.diso.copy(), Perturbed_discharge[0], "Discharge", "m³/s", "cdf_discharge.png")
        plot_cdf_variable(optn_args.ddho.copy(), Perturbed_depth[0], "Depth", "m", "cdf_depth.png")

    plot_rating_curve(diso, ddho, "rating_curve_reference.png")
    plot_rating_curve(Perturbed_discharge[0], Perturbed_depth[0], "rating_curve_perturbed.png")
    plot_rating_curve(df.index, Perturbed_depth[0], "rating_curve_time_series_perturbed.png")
    plot_rating_curve(df.index, Perturbed_discharge[0], "rating_curve_time_series_perturbed.png")
    ...
    plot_sims_curves(df.index, selected_discharges, selected_sims, "Reference Discharge and 3 Random Simulated Discharges", "Random_3_Simulations_Discharge.png")
...
def multi(ags):
    ...
    diso_from_ddho = build_diso_from_ddho(df)
    # ddho perturbieren
    ddho_new = perturbed_ddho(df["ddho__ref"].values)
    # Now get new discharge profile dependent on the new perturbed depth
    diso_new = np.array([diso_from_ddho(d) for d in ddho_new])
    
    # Calibrate model with changed discharge -> have to rewrite it
    best_params, best_ofv, best_metrics, best_sim = CalibrateModel(diso_new, ags, optn_args.modl_objt) 
    ofv_old, _, _ = objective_function(Best_Parameter_Assignment1, optn_args.modl_objt, diso_new)
...
def perturbed_ddho(ddho):
    ...
    perturbation = np.random.choice(values, size=len(ddho))
...
def CalibrateModel(ref_diso, ags, model):
    ...
    res = differential_evolution(
        ...
        popsize=popsize,
        tol=1e-4,
        mutation=(0.5, 1.0), # Increased upper bound for mutation to encourage more exploration
    ...
    save_ofv = min(local_res.fun , res.fun)#best_solution["ofv"] if np.isfinite(best_solution["ofv"]) else best_ofv
...
def plot_sims_curves(time_index, observed, simulations, title, out_png):
    ...
    for i, (obs, sim) in enumerate(zip(observed, simulations)):
        plt.plot(time_index, obs, label=f"Target {i+1}", color=f'C{i}', linestyle='--', linewidth=1.5, alpha=0.8)
        plt.plot(time_index, sim, label=f"Sim {i+1}", color=f'C{i}', linestyle='-', linewidth=1.0, alpha=0.8)
    plt.xlabel("Time")
...
def plot_rating_curve(depth, discharge, out_png):
...
def plot_cdf_ofvs(ofvs_ref, ofvs_perturbed, out_png):
    ...
    plt.plot(sorted_ref, p_ref, label="Reference Discharge", marker='o', linestyle='-', markersize=4)
    plt.plot(sorted_perturbed, p_perturbed, label="Perturbed Discharge", marker='s', linestyle='--', markersize=4)
...
def plot_cdf_variable(ref_data, perturbed_data, var_name, unit, out_png):
    ...
    sorted_perturbed = np.sort(perturbed_data)
    p_ref = np.arange(1, len(sorted_ref) + 1) / len(sorted_ref)
    p_perturbed = np.arange(1, len(sorted_perturbed) + 1) / len(sorted_perturbed)
    plt.plot(sorted_ref, p_ref, label=f"Reference {var_name}", marker='o', linestyle='-', markersize=4)
    plt.plot(sorted_perturbed, p_perturbed, label=f"Perturbed {var_name}", marker='s', linestyle='--', markersize=4)
...
def plot_scatter_ofvs_precipitation(OFVs, OFVs_REF, out_png):
    ...
    plt.xlabel("OFV when recalibrating after perturbation of depth and hence discharge")
    plt.ylabel("OFV when using parameters from assignment 1 and modified discharge")
