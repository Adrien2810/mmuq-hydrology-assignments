...
def main():
    ...
    if Perturbed_discharge:
        ...
        ), Perturbed_discharge[0], "Discharge", "m³/s", "cdf_discharge.png")
        plot_cdf_variable(optn_args.ddho.copy(),
                          Perturbed_depth[0], "Depth", "m", "cdf_depth.png")
...
def plot_cdf_variable(ref_data, perturbed_data, var_name, unit, out_png):
    ...
    plt.plot(sorted_ref, p_ref,
             label=f"Reference {var_name}", marker='o', linestyle='-', markersize=4)
    plt.plot(sorted_perturbed, p_perturbed,
             label=f"Perturbed {var_name}", marker='s', linestyle='--', markersize=4)
