...
def main():
    ...
    mprg_pool_size = 4
    #====
    # Build interpolating function for diso from ddho using polynomial fit
    fit_first, fit_second, a, c = build_diso_from_ddho(df)
    diso_interpolated = get_discharge_from_ddho(df["ddho__ref"].values, fit_first, fit_second, a, c)
    ...
    _, ofv_interpol, _ , _= CalibrateModel(get_discharge_from_ddho(df["ddho__ref"].values, fit_first, fit_second, a, c), (0, optn_args), modl_objt)
...
def multi(ags):
    ...
    diso_new = get_discharge_from_ddho(ddho_new, optn_args.fit_first, optn_args.fit_second, optn_args.a, optn_args.c)
...
def get_discharge_from_ddho(ddho, fit_first, fit_second, a, c):
    xs = 430.5
    ddho = np.asarray(ddho, dtype=float)
    p1 = np.poly1d(fit_first)
    p2 = np.poly1d(fit_second)
    return np.where(ddho <= xs, p1(ddho), a * p2(ddho) + c)
