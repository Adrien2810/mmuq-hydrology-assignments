...
def plot_convergence(best_list, out_png="de_convergence.png"):
    ...
    plt.yscale("log")
    ax.set_xlabel("Number of Generation"); ax.set_ylabel("Objective (1 - NSE) on log scale")
...
def plot_param_scatter(eval_params, eval_ofv, param_names, bounds, out_dir="."):
    ...
    for i, (name, (lo, hi)) in enumerate(zip(param_names, bounds)):
        ...
        ax.set_ylim(np.min(eval_ofv_finite) * 1.8 , 1)
...
def main():
    ...
    data_dir = Path(r'C:\Users\adrie\Documents\Python\MMUQ Hydrology\mmuq-hydrology-assignments\Assignment 1\data') 
    #data_dir = Path.home() / ... / ...
